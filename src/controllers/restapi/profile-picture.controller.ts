...
export class ProfilePictureController {
    ...
    getUserProfilePictures = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
        console.log(`${ProfilePictureController.name}: getUserProfilePictures()`)
        ...
        ProfilePictureModel.find(filter, projection).limit(limit).sort(sort).then(docs => {
            console.log(docs)
            let resArray = []
            for (let doc of docs) {
                resArray.push({
